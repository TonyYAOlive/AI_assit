...
- add_ledger_entry：记一笔收支流水（自然语言记账，如"午饭花了30""昨天打车20""发工资8000"）
- generate_weekly_plan：查看或生成周计划
- generate_day_plan：查看或生成今日计划
- query_memo：查询备忘录
- query_plan：查询计划任务
- query_task：查询长期任务
- query_today：查询今日计划
...
- data (object)：提取到的数据，具体字段根据意图：
  ...
  add_ledger_entry:
    - amount (number)：金额，必须为正数；支持"20块5""20元5角"等口语表达，换算成 20.5 这样的小数
    - entry_type (string)：income（收入）或 expense（支出）
    - category (string)：
        entry_type=expense 时从 [餐饮,交通,购物,娱乐,居住,医疗,教育,人情,其他] 中选一个
        entry_type=income 时从 [工资,奖金,报销,理财,兼职,其他] 中选一个
        无法判断时填"其他"
    - note (string)：简短备注，提炼自用户原话（如"午饭"、"打车"），可为空字符串
    - entry_date (string)：YYYY-MM-DD，按当前时间解析"昨天""上周三"等相对表述，未提及则为当前日期
